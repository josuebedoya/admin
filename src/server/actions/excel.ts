...
import {ProductExcel} from "@/server/store/productSnapshotRepository/getById";
...
const groupByShelf = (products: ProductExcel[]): ResGroupByShelf => {
...
const getShelfLabel = (product: ProductExcel): string => {
...
const generateReport = async (idReport: string, alias?: string) => {
  ...
  try {
    ...
    const grouped = groupByShelf(products.items as ProductExcel[])
