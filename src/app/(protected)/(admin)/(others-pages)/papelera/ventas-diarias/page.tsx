import ComponentCard from "@/components/common/ComponentCard";
import PageBreadcrumb from "@/components/common/PageBreadCrumb";
import {Metadata} from "next";
import {dictionary} from "@/dictionary";
import TableDeletedDailySales from "@/components/wastebasket/tableDeletedDailySales";
import {getDailySales} from "@/server/store/dailySaleRepository";

export const metadata: Metadata = {
  title: "Ventas diarias eliminadas - Admin",
  description: "Administra las ventas diarias eliminadas en la papelera de reciclaje. Restaura las ventas diarias según sea necesario.",
};

type PageProps = {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
};

export default async function PageDailySales({searchParams}: PageProps) {
  const params = await searchParams;
  const page = Number(params.page) || 1;
  const pageSize = Number(params.pageSize) || 10;

  const {data: dailySales, error, message} = await getDailySales({page, pageSize, getDeleted: true});

  return (
    <div>
      <PageBreadcrumb pageTitle={dictionary.admin.wastebasket.sales.title}/>
      <div className="space-y-6">
        <ComponentCard title={dictionary.admin.wastebasket.sales.description}>
          {error ? (
            <div className="p-4 bg-red-100 text-red-700 rounded">
              {dictionary.msg[message as keyof typeof dictionary.msg] || 'Error al cargar los datos.'}
            </div>
          ) : (
            <TableDeletedDailySales
              items={dailySales.items || []}
              totalAmount={dailySales.count || 0}
              currentPage={page}
              pageSize={pageSize}
            />
          )}
        </ComponentCard>
      </div>
    </div>
  );
}
